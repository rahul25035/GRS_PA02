...
void *handle_client(void *arg) {
    ...
    int one = 1;
    setsockopt(c, SOL_SOCKET, SO_ZEROCOPY, &one, sizeof(one));
...
int main(int argc, char *argv[]) {
    ...
    if (argc != 3) {
        write(
            2,
            "Usage: ./server_a3 <field_size> <max_threads>\n",
            sizeof("Usage: ./server_a3 <field_size> <max_threads>\n") - 1
        );
        return 1;
    }
